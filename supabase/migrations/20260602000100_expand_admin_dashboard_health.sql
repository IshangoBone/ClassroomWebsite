drop function public.get_admin_dashboard_summary();
...
as $$
    ...
    select
        ...
        (select count(*) from public.lesson_submissions where status = 'draft') as draft_submissions,
        (
            select count(*)
        ...
        (select count(*) from public.files where status = 'deleted') as deleted_files,
        (
            select count(*)
