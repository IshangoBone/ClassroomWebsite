drop function if exists public.get_admin_platform_analytics_drilldown(text, integer, integer);
drop function if exists public.get_admin_platform_analytics_drilldown(text, integer);
...
as $$
    ...
    new_user_records as (
        ...
        where created_at >= now() - (normalized.days * interval '1 day')
    ),
